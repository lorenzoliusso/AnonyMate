...
# BUG: replace path-venv/lib/python3.10/site-packages/pdfrw/objects/pdfdict.py line 147 with: del self[key] insetad of del self[name]
...
def safe_tokenize(stream):
    """Safely tokenize a single PDF content stream."""
    ...
    try:
        ...
        # Handle tokenization errors gracefully and log the issue
        print(f"Warning: Error tokenizing stream due to '{e}'. Skipping this stream.")
        return
...
def tokenize_streams(streams):
	...
	for stream in streams:
		#tokens = PdfTokens(stream)
		for token in safe_tokenize(stream):
	...
	for page in document.pages:
		...
		def make_mutable_string_token(token):
			if isinstance(token, PdfString):
				try:
					# Attempt to convert the token to bytes and create a TextToken
					token = TextToken(token.to_bytes(), current_font)
				except ValueError as e:
					print(f"Warning: Skipping invalid PDF string {token}. Error: {e}")
					return None  # Skip this token if invalid

				# Cache glyphs for the current font if available
